    ...
    <el-card shadow="never">
      ...
      <div v-for="q in questions" :key="q.id" class="question-block">
        ...
        <el-slider
          ...
          show-stops
        />
        <el-radio-group v-else-if="q.type === 'ternary'" v-model="answers[q.id]">
...
async function onSubmit() {
  ...
  saveSubmission('questionnaire', { answers: { ...answers }, durationSec: sec })
  endTaskTimer('questionnaire')
  trackEvent('questionnaire', 'submit', { durationSec: sec, sessionId: getCurrentSessionId() })
  try {
    await runStepSubmit({
      router,
      submit: () =>
