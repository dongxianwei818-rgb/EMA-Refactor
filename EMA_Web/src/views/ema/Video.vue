<template>
  <div class="ema-task-page">
    <el-card shadow="never">
      ...
      <p class="hint">建议录制 {{ minSec }}–{{ maxSec }} 秒 · 正脸自拍 · 可选择不露脸</p>

      <el-alert v-if="intervalHint" type="info" :title="intervalHint" show-icon class="interval-alert" />

      <div class="video-preview">
        <video v-show="phase === 'recording'" ref="videoEl" autoplay muted playsinline class="preview" />
        <div v-if="phase !== 'recording'" class="preview-placeholder">
          {{ recordText }}
        </div>
      </div>

      <el-checkbox v-model="hideFace" @change="onHideFaceToggle">不露脸（使用后置摄像头提示）</el-checkbox>
      ...
      <div class="btn-row">
        ...
        <el-button v-if="phase !== 'submitting'" @click="onSkip">跳过本次</el-button>
      </div>
    </el-card>
  </div>
</template>
...
async function submitAndNext(file, sec) {
  ...
  markTaskDone('video')
  markVideoDone()
  saveSubmission('video', { duration: sec, hideFace: hideFace.value, question })
  endTaskTimer('video')
...
async function skipToNext() {
  ...
  try {
    await runStepSubmit({
      router,
      submit: () => submitVideoSkipLog(at, getCurrentSessionId(), getTodayKey()),
      successToast: '已进入下一步',
    })
  } finally {
    submitting.value = false
