          ...
          <el-form label-position="top">
            <el-form-item v-for="f in section.fields" :key="f.id">
              ...
              <el-input
                ...
                placeholder="请输入"
              />
              <el-radio-group v-else-if="f.type === 'radio'" v-model="form[f.id]" class="radio-group">
                <el-radio v-for="opt in f.options" :key="opt" :value="opt">{{ opt }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-form>
        </el-card>
...
import { bindResearchFromBaseline, hasBaseline, isResearchBound, saveBaseline } from '../utils/ema'
...
function validate() {
  const req = ['researchId', 'age', 'gender', 'grade', 'major', 'onlyChild', 'housing']
  for (const id of req) {
    if (!form[id]) {
  ...
  const items = [
    'course_pressure', 'exam_pressure', 'gpa_pressure', 'job_pressure',
    'sleep_habit', 'exercise_freq', 'social_freq',
    'phq9_1', 'phq9_2', 'gad7_1', 'gad7_2',
  ]
